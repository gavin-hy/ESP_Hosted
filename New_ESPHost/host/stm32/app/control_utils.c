...
//add start 2-22
const char * const wifi_auth_string_list[]={
    "OPEN",
    "WEP",
    "WPA_PSK",
    "WPA2_PSK",
	...
	switch(app_resp->msg_id) {
		...
		case CTRL_RESP_GET_MAC_ADDR: {
			...
			if (!list) {
				...
				//wifi_auth_string_list = list->encryption_mode;

				for (i=0; i<w_scan_p->count; i++) {
					printf("%d) ssid \"%s\" bssid \"%s\" rssi \"%d\" channel \"%d\" auth mode \"%s\" \n\r",\
							i, list[i].ssid, list[i].bssid, list[i].rssi,
							list[i].channel, wifi_auth_string_list[list[i].encryption_mode] );
				}
				msleep(1);
			}
			break;
