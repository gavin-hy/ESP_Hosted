...
//add test 2-26
static osSemaphoreId task_Sem_1_2;

static osThreadId task_id_1 = 0;
static osThreadId task_id_2 = 0;

/** function definition **/
uint16_t task1_cnt =0;
void Task1(void const* pt)
{
    osSemaphoreDef(TS);
    task_Sem_1_2 = osSemaphoreCreate(osSemaphore(TS) , 1);
    assert(task_Sem_1_2);
    
    for(;;){
        
       if(task_Sem_1_2){
            osSemaphoreRelease(task_Sem_1_2);
       }          
       task1_cnt++ ;
       printf("Task1 running count is %d \r\n",task1_cnt);
       vTaskDelay(1000);
    }
...
void Task2(void const*pt)
{
    for(;;){
        
       if(osSemaphoreWait(task_Sem_1_2,portMAX_DELAY)!=osOK) {
//           printf("could not obtain task_Sem_1_2!\r\n");
      }
       
           task2_cnt++ ;
           printf("Task2 running count is %d \r\n",task2_cnt);
           vTaskDelay(800);
...
void transport_init(void(*transport_evt_handler_fp)(uint8_t))
{
	...
	printf("Start Create TX&RX Task!");
...
//    osThreadDef(task1_thread,Task1,osPriorityNormal,0,128);
//    task_id_1 = osThreadCreate(osThread(task1_thread),NULL);
//    assert(task_id_1);
//    
//    osThreadDef(task2_thread,Task2,osPriorityAboveNormal,0,128);
//    task_id_2 = osThreadCreate(osThread(task2_thread),NULL);
//    assert(task_id_2);
...
{
	if ( (GPIO_Pin == ESP_DATA_READY_Pin) ||
	     (GPIO_Pin == ESP_HANDSHAKE_Pin) )
	{
...
static void check_and_execute_spi_transaction(void)
{
	...
	gpio_handshake = HAL_GPIO_ReadPin(ESP_HANDSHAKE_GPIO_Port,
			ESP_HANDSHAKE_Pin);

	/* data ready line SET -> slave wants to send something */
	gpio_rx_data_ready = HAL_GPIO_ReadPin(ESP_DATA_READY_GPIO_Port,
			ESP_DATA_READY_Pin);
...
static stm_ret_t spi_transaction_v2(uint8_t * txbuff)
{
	...
	/* Allocate rx buffer */
	rxbuff = (uint8_t *)pvPortMalloc(MAX_SPI_BUFFER_SIZE);
	assert(rxbuff);
	memset(rxbuff, 0, MAX_SPI_BUFFER_SIZE);
	...
	if(!txbuff) {
		 ...
		 */
		txbuff = (uint8_t *)pvPortMalloc(MAX_SPI_BUFFER_SIZE);
		assert(txbuff);
		memset(txbuff, 0, MAX_SPI_BUFFER_SIZE);
	}
